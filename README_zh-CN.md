...
**SpineDigest 用来为 AI Agent 构建可携带的 LLM Wiki 归档。** 它把长文档导入 `.sdpub`，再让 Agent 通过 CLI 搜索、浏览、阅读 source fragment、沿图链接导航，并在需要时导出投影视图，而不是解压归档读内部文件。

对 Agent 来说，探索 `.sdpub` 有三条主线：

- **搜索模式：** 用 `find` 做按空白拆分的多关键词发现，用 `grep` 检查连续精确文本。
- **结构模式：** 用 `list` 获取有界对象集合，用 `page` 打开带局部导航的单个详情对象。
- **阅读模式：** 用 `read` 将 chapter、summary、fragment 或 node 以连续纯文本输出。
