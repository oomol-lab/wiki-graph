...
export class SpineDigest {
  ...
  public async readSerialSummary(serialId: number): Promise<string> {
    return await this.#document.openSession(async (document) => {
      ...
      if (record === undefined) {
        throw new Error(
          `No completed summary exists for id ${serialId}. Use \`wikigraph chapter list <archive.sdpub>\` to discover chapter ids, then \`wikigraph get wikigraph://<archive.sdpub>/chapter/${serialId}/summary/\` after summary is ready.`,
        );
      }
      ...
      if (summary === undefined) {
        throw new Error(
          `Chapter ${serialId} summary is missing. Run \`wikigraph queue add <archive.sdpub> --chapter ${serialId} --task reading-summary --accept-cost\` before export, or inspect the archive with \`wikigraph index <archive.sdpub>\`.`,
        );
      }
