...
在 URI 中显式选择 search lens：`/chunk` 用于 Reading Graph 结构，`/summary` 用于快速概览，`/source` 用于原文措辞，`/entity` 和 `/triple` 用于 Knowledge Graph 对象。使用 `wkg://book.sdpub/chapter/3/entity` 这类 scoped chapter lens URI、`--limit`、`--cursor` 控制检索范围。
...
7. 用户需要围绕已知 chunk、entity 或 triple 打包确定性上下文时，使用 `wikigraph <graph-object-uri> pack`。
